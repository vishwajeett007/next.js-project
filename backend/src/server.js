...
app.post('/api/budget/sync', async (req, res) => {
    ...
    try {
        ...
        if (pool) {
            await pool.query('INSERT INTO budgets (data, updated_at) VALUES ($1, $2) ON CONFLICT (id) DO UPDATE SET data = $1, updated_at = $2', [JSON.stringify(budget), new Date()]);
        }
...
app.get('/api/budget/latest', async (req, res) => {
    try {
        if (pool) {
